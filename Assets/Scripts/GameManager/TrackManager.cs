...
public class TrackManager : ReRegisterMonoSingleton
{
    ...
    [SerializeField] CharacterInputController m_characterController;
    ...
    public CharacterInputController characterController { get { return m_characterController; } }
    ...
    //Missions Manager
    // for PickupMission
    [HideInInspector] public int previousCoinAmount;
    // for OBSTACLE_JUMP
    [HideInInspector] public Obstacle m_Previous;
    [HideInInspector] public Collider[] m_Hits;
    [HideInInspector] public int k_HitColliderCount = 8;
    [HideInInspector] public Vector3 k_CharacterColliderSizeOffset = new Vector3(-0.3f, 2f, -0.3f);
    //for sliding Mission
    ...
    {
        m_characterController.StartMoving();
        m_IsMoving = true;
        if (isRestart)
            m_Speed = minSpeed;
    }
    ...
    {
        m_characterController.character.animator.Play(s_StartHash);
        ...
        if (m_Rerun)
        {
            ...
            m_characterController.characterCollider.SetInvincible();
        }

        m_characterController.StartRunning();
        StartMove();
    }
    ...
    public IEnumerator Begin(GamePlayManager manager)
    {
        ...
        if (!m_Rerun)
        {
            ...
            // Spawn the player
            var op = Addressables.InstantiateAsync(m_PlayerData.characters[m_PlayerData.usedCharacter],
                Vector3.zero,
                Quaternion.identity);
            yield return op;
            if (op.Result == null || !(op.Result is GameObject))
            {
                Debug.LogWarning(string.Format("Unable to load character {0}.", m_PlayerData.characters[m_PlayerData.usedCharacter]));
                yield break;
            }
            Character player = op.Result.GetComponent<Character>();

            player.SetupAccesory(m_PlayerData.usedAccessory);
            ...
            if (m_IsTutorial)
                ...
                m_CurrentThemeData = ThemeDatabase.GetThemeData(m_PlayerData.themes[m_PlayerData.usedTheme]);
            ...
            m_characterController.gameObject.SetActive(true);
            m_characterController.coins = 0;
            m_characterController.premium = 0;
    ...
    void Update()
    {
        ...
        Transform characterTransform = m_characterController.transform;
