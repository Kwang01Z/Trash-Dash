...
public class CharacterDatabase
{
    ...
    static public IEnumerator LoadDatabase()
    {
        if (m_CharactersDict == null)
        {
            ...
            m_Loaded = true;
            Debug.Log("Load Character Database Successfull!");
        }
    }
    static public List<string> GetListCharName()
    {
        List<string> listName = new List<string>();
        if(loaded)
        foreach (KeyValuePair<string, Character> keyValue in m_CharactersDict)
        {
